(function () {
  ...
  if (profBtn && profMenu) {
    profBtn.addEventListener("click", e => { e.stopPropagation(); profMenu.classList.toggle("active"); });
    document.addEventListener("click", e => {
      if (!profMenu.contains(e.target) && !profBtn.contains(e.target)) profMenu.classList.remove("active");
    });
  }
  ...
  function updateCartUI() {
    ...
    body.innerHTML = cart.map(item => `
      <div class="drawer-item">
        ${item.image ? `<img src="${item.image}" alt="${item.name}" class="drawer-item-img">` : `<div class="drawer-item-img" style="display:flex;align-items:center;justify-content:center;color:var(--sage)"><i class="fas fa-seedling"></i></div>`}
  ...
  let wishlistIds = new Set(
    (window.__WISHLIST_IDS__ || []).map(Number)
  );
  ...
  if (revTrack) {
    ...
    revTrack.addEventListener("touchend",   e => { const dx = e.changedTouches[0].clientX - rtx; if (Math.abs(dx) > 40) slideRev(dx < 0 ? 1 : -1); }, { passive: true });
  ...
  window.addEventListener("resize", () => { revIdx = 0; if (revTrack) revTrack.style.transform = "translateX(0)"; }, { passive: true });
  ...
  document.querySelectorAll(".filter-tab").forEach(tab => {
    tab.addEventListener("click", function () {
      ...
      document.querySelectorAll(".product-card").forEach(card => {
        ...
        const show = filter === "all" || (filter === "sale" && hasSale) || (filter === "instock" && inStock);
        card.style.transition = "opacity .25s,transform .25s";
        if (show) { card.style.opacity = "1"; card.style.transform = ""; card.style.display = ""; }
        else       { card.style.opacity = "0"; card.style.transform = "scale(.95)"; setTimeout(() => { if (card.style.opacity === "0") card.style.display = "none"; }, 260); }
      });
    });
  });
